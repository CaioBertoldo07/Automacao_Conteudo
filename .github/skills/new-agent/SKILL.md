...
agents/
  ...
  content.agent.ts   ← legendas + hashtags (caption + hashtags[])
...
### 1. Definir a interface de contexto (input)
...
### 2. Definir o schema Zod de output
...
### 3. Implementar extrator de JSON (reutilizar padrão)
...
### 4. Construir o prompt em função separada
...
### 5. Implementar a função principal
...
export async function generate<Funcao>(ctx: <Funcao>Context): Promise<<Funcao>Output> {
  ...
  try {
    ...
    throw Object.assign(
      new Error("Resposta do agente não é JSON válido."),
      { statusCode: 502 }
    );
  }
  ...
  if (!result.success) {
    throw Object.assign(
      new Error("Estrutura de resposta do agente inválida."),
      { statusCode: 502 }
    );
  }
...
- Sempre usar `extractJson()` antes de `JSON.parse()` (a IA pode embrulhar em markdown code blocks)
- Exportar o tipo de output derivado do schema Zod: `export type X = z.infer<typeof schema>`
- A constante `MODEL` sempre lê de `process.env.GOOGLE_MODEL` com fallback para `"gemini-2.5-flash"`

## Integrar ao worker (se assíncrono)

Se a geração for pesada, integrar via BullMQ — ver skill `new-worker`.
O agente não deve ser chamado diretamente nas rotas.

## Adapters para mídia (imagem/vídeo)

Para `image.adapter.ts` e `video.adapter.ts`, o padrão difere: a resposta não é JSON,
mas dados binários (base64 ou arquivo). Nesses casos não há `extractJson` / Zod,
mas o tratamento de erros `502`/`503` permanece idêntico.
...
- [ ] Validação `safeParse` com erro `502` em caso de falha
